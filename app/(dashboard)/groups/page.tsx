...
export default function GroupsPage() {
  ...
  async function copyLink() {
    ...
    try {
      ...
      setCopied(true);
      // Reset the tick mark after 2 seconds
      setTimeout(() => {
        setCopied(false);
      }, 2000);
    } catch (err) {
          ...
          {link && (
            <div className="ml-2 flex flex-1 items-center gap-3 rounded border border-slate-800 bg-slate-900/40 px-3 py-2">
              ...
              <button
                type="button"
                onClick={copyLink}
                className="flex items-center gap-1.5 rounded bg-slate-800 px-3 py-1 text-xs text-slate-200 hover:bg-slate-700 transition-colors"
              >
                {copied ? (
                  <>
                    <svg
